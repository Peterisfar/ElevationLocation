...
name = "10_55_53"
ImageData = read_data_to_txt("./data/locdata/test/{}_res.txt".format(name))  # 24 --->640  (list)
...
for i, skyline_dem in enumerate(Demfilenames):
    if i == 2948:
        ...
        S, yaws = get_close_S(shape_d, corner_d_x[Sc], portion, 100)
        Sc, Cost = distance(S, T, model, device)
        show_match_result_dem_and_img(S, T, Sc, Cost)
        print(time.time()-time1)

        # step 6 : 异常值过滤
        Cost[Sc] = Cost[Sc] if np.sum((np.array(S[Sc])[:, 1] < 5)==True) < 100 else 0.
...
# print(df)
df.to_csv("./data/locdata/{}_new.csv".format(name))